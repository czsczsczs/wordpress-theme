    ...
    class WPCOM_Notifications{
        ...
        function send(){
            ...
            if (check_ajax_referer('wpcom_send_notifications', 'wpcom_send_notifications_nonce', false)) {
                ...
                if($to_user==='') $res['error'] = '發送對象不能爲空';
                if($title==='') $res['error'] = '標題不能爲空';
                if($content==='') $res['error'] = '內容不能爲空';
        ...
        function admin_page(){ ?>
            <div class="wrap">
                ...
                <a href="javascript:;" class="page-title-action j-send-notifications">發通知</a>
                <form method="post">
                    <?php
                ...
                if($post && isset($post->ID) && in_array($post->post_type, array('post', 'qa_post'))){
                    ...
                    if($comment->comment_parent){
                        ...
                        if($parent->user_id && $parent->user_id !== $comment->user_id){
                            ...
                            $content .= '更多詳情請訪問：<a href="'.get_permalink($post->ID).'" target="_blank">'.get_permalink($post->ID).'</a>';
                        }
        ...
        function post_notify( $new_status, $old_status, $_post ){
            ...
            $content = $content = '親愛的用戶：你好！<br>';
            if($new_status==='publish'){ // 审核通过
                $title = '文章《'.$post->post_title.'》審覈通過';
                $content .= '恭喜，你的文章《<a href="'.get_permalink($post->ID).'" target="_blank">'.$post->post_title.'</a>》已經審覈通過，感謝你對本站的支持！';
            }else if($old_status==='pending' && $new_status==='draft'){ // 审核未通过
                $title = '文章《'.$post->post_title.'》審覈未通過';
                $content .= '很抱歉你的文章《<a href="'.get_edit_link($post->ID).'" target="_blank">'.$post->post_title.'</a>》審覈未通過，感謝你對本站的支持！';
            }else if($old_status==='pending' && $new_status==='trash'){ // 删除
                $title = '文章《'.$post->post_title.'》審覈未通過';
                $content .= '很抱歉你的文章《'.$post->post_title.'》審覈未通過，感謝你對本站的支持！';
            }
        ...
        function follow_notify( $user_id, $followed ){
            ...
            $title = $user->display_name . ' 關注了你';
            $content = $content = '親愛的用戶：你好！<br>';
            $content .= $user->display_name . ' 關注了你，點擊查看Ta的信息:<a href="'.wpcom_author_url( $user_id, $user->user_nicename ).'" target="_blank">'.wpcom_author_url( $user_id, $user->user_nicename ).'</a>';
