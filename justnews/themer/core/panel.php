...
class WPCOM_Panel{
    ...
    public function panel_active(){
        if(isset($_POST['email'])){
            ...
            if($err==false){
                ...
                $body = array('email'=>$email, 'token'=>$token, 'version'=>THEME_VERSION, 'home'=>get_home_url(), 'themer' => FRAMEWORK_VERSION, 'hash' => $hash_token);
    ...
    public function check_version(){
        $body = array('version'=>THEME_VERSION,'email' => get_option('izt_theme_email'),'home' => get_home_url(),'themer' => FRAMEWORK_VERSION);
    ...
    private function theme_update(){
        ...
        if($version && $current_ver && version_compare($version, $current_ver) < 0){
            ...
            $token = get_option('izt_theme_token');
            if( $email &&  $token ) {
                $body = array('email'=>$email, 'token'=>$token, 'version'=>$current_ver, 'home'=>get_home_url(), 'themer' => $this->framework_version());
    ...
    public function check_update($value){
        ...
        if ( !$this->automaticCheckDone ) {
            $body = array('email' => get_option('izt_theme_email'), 'token' => get_option('izt_theme_token'), 'version' => THEME_VERSION, 'home' => get_home_url(), 'themer' => FRAMEWORK_VERSION);
